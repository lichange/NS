...
namespace NS.Component.Utility
{
    public class ZipHelper
    {
        ...
        /// <param name="directoryPath">解压后的文件夹路径</param>
        public static void ExtractZip(string filePath, string directoryPath)
        {
            if (System.IO.File.Exists(filePath))
            {
                var zip = ZipFile.Read(filePath);
        ...
        /// 创建一个压缩包
        /// </summary>
        /// <param name="filePath">新建压缩包文件路径</param>
        /// <param name="filePaths">待压缩的文件路径集合</param>
        public static void CreateZip(string filePath, IList<string> filePaths)
        {
            if (System.IO.File.Exists(filePath))
            {
                var zip = new ZipFile(filePath);

                if (filePaths != null && filePath.Count() > 0)
                    foreach (var itemFilePath in filePaths)
                    {
                        if (string.IsNullOrEmpty(itemFilePath))
                            continue;
                        zip.AddFile(itemFilePath);
                    }
                zip.Save();
            }
        }
    ...
    internal class ZipFile : IEnumerable<ZipEntry>, IDisposable
    {
        ...
        private Stream ReadStream
        {
            get
            {
                return _readStream ?? (_readStream = File.OpenRead(Name));
            }
        }
        /// <summary>
        ...
        private FileStream WriteStream
        {
            get
            {
                return _writeStream ?? (_writeStream = new FileStream(Name, FileMode.CreateNew));
            }
        }
        /// <summary>
        ...
        public ZipEntry this[string fileName]
        {
            get
            {
                return _entries.FirstOrDefault(entry => entry.FileName == fileName);
            }
        }
    ...
    internal class ZipEntry
    {
        ...
        public string DirectoryNameOverride
        {
            get;
            set;
        }
        ...
        private static bool ReadHeader(Stream stream, ZipEntry entry)
        {
            ...
            if ((entry.BitField & 0x0008) == 0x0008)
            {
                ...
                if (sizeOfDataRead != entry.CompressedSize)
        ...
        internal static ZipEntry Create(byte[] bytesToCompress, string fileName, DateTime fileDate)
        {
            ...
            {
                FileName = fileName,
                BytesToCompres = bytesToCompress,
    ...
    /// </summary>
    internal class SharedZipFunctionality
    {
        /// <summary>
        ...
        protected internal static int ReadSignature(Stream signatureStream)
        {
            ...
            if (counter != signatureBytes.Length)
                ...
                if (counter != 0)
                {
                    for (int counter2 = 0; counter2 < counter; counter2++)
                        if (batch[counter2] == targetBytes[3])
                        {
                            ...
                            if (!success)
                                signatureStream.Seek(-3, SeekOrigin.Current);
