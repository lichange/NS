...
namespace NS.Component.Utility
{
    public class ReflectHelper
    {
        ...
        public static Func<object, DateTime> LmdDateTimeGet(Type entityType, string propName)
        {
            #region 通过方法取值
            var p = entityType.GetProperty(propName);
            //对象实例
            ...
            return Expression.Lambda<Func<object, DateTime>>(body, param_obj).Compile();
            #endregion
        ...
        public static object GetPropertyValueEmit(object entity, string propertyName)
        {
            var property = GetProperty(entity.GetType(), propertyName);
            var lamdaExp = LmdGet(entity.GetType(), propertyName);
            return lamdaExp(entity);
            //if (property == null)
            //    throw new ArgumentNullException("属性对象为空！");
