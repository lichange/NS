...
namespace NS.Component.Utility
{
    ...
    public sealed class SqlHelper
    {
        ...
        {
            var key = "ConStr";
            var keyItem = NS.Framework.Config.PlatformConfig.ServerConfig.KeyValueSettings.KeyValueItems.Where(pre => pre.Key.ToLower() == key.ToLower()).FirstOrDefault();
            if (keyItem != null)
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                foreach (PropertyInfo pi in propertys)
                {
                    ...
                    if (dt.Columns.Contains(tempName))
                    {
                        ...
                        {
                            DateTime tempInt = DateTime.Parse("1900-01-01 00:00:00");
                            pi.SetValue(t, DateTime.TryParse(value.ToString(), out tempInt) ? tempInt : tempInt, null);
                        }
                        else if (pi.PropertyType.FullName == "System.Double")
                        {
                            Double tempInt = 0;
                            pi.SetValue(t, Double.TryParse(value.ToString(), out tempInt) ? tempInt : tempInt, null);
                        }
                        else if (pi.PropertyType.FullName == "System.Int32")
                        {
                            int tempInt = 0;
                            pi.SetValue(t, int.TryParse(value.ToString(), out tempInt) ? tempInt : tempInt, null);
                        }
                        else
                        {
