...
namespace NS.Component.Data.Impl
{
    ...
    /// </summary>
    internal class NHibernateDataAccessor : IDataAccessor
    {
        ...
        {
            return NHibernateDataAccessorManager.GetSession(config);
        }

        private ISession GetSingletonSession()
        {
            if (singletonSession == null)
            {
                ...
                {
                    if (singletonSession == null)
                        singletonSession = NHibernateDataAccessorManager.GetSession(config);
                }
            }
                                    ...
                                    case ChangeType.Update:
                                        if (Session.Contains(item.Key))
                                        {
                                            ...
                                            if (tempIdentifier != null)
                                            {
                                                ...
                                                if (tempT != null)
                                                    //var entityData= Session.SessionFactory.GetClassMetadata(data.GetType());
                                                    Session.Replicate(item.Key.GetType().Name, item.Key, ReplicationMode.LatestVersion);
                                            }
                                            //var tempData = Session.Merge(data);
                                        }
                                        else
        ...
        {
            var Session = GetSingletonSession();
            try
            {
                var tempData = data;
                if (Session.Contains(data))
