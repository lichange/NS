...
namespace NS.Component.Memcache.Caching.Memcached
{
    ...
    /// 
    ///		<locator factory="NS.Component.Memcache.Caching.Memcached.VBucketNodeLocatorFactory" configFile="vbucket.json" />
    /// 
    /// </remarks>
    public class VBucketNodeLocatorFactory : IProviderFactory<IMemcachedNodeLocator>
    {
        ...
        void IProvider.Initialize(Dictionary<string, string> parameters)
        {
            ...
            var tmp = JsonConvert.DeserializeObject<int[][]>(json); //JavaScriptSerializer().Deserialize<int[][]>(json);
