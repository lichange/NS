...
namespace NS.Framework.Utility.Reflection
{
    public class ReflectHelper
    {
        ...
        public static Func<object, object> LmdGet(Type entityType, string propName)
        {
            #region 通过方法取值
            var p = entityType.GetProperty(propName);
            //对象实例

            ParameterExpression param_obj = null;

            //if (p.PropertyType == typeof(DateTime))
            //    param_obj = Expression.Parameter(typeof(DateTime), "obj");
            //else
            param_obj = Expression.Parameter(typeof(object), "obj");
        ...
        public static Action<object, object> LmdSet(Type entityType, string propName)
        {
            var p = entityType.GetProperty(propName);
            //对象实例
        ...
        public static Assembly GetAssembly(string entityTypeAssemblyName)
        {
            if (string.IsNullOrEmpty(entityTypeAssemblyName))
                return null;

            //先从当前程序域中筛选，如果不存在，再通过load机制，装入内存
            var tempServiceAssemly = System.AppDomain.CurrentDomain.GetAssemblies().Where(pre => pre.GetName().Name == entityTypeAssemblyName).FirstOrDefault();

            if (tempServiceAssemly == null)
            {
                var assemblyPath = System.IO.Path.Combine(NS.Framework.Config.PlatformConfig.ServerConfig.IOCSetting.AssemblyFilePath, entityTypeAssemblyName);

                if (!System.IO.Directory.Exists(NS.Framework.Config.PlatformConfig.ServerConfig.IOCSetting.AssemblyFilePath))
                {
                    assemblyPath = System.IO.Path.Combine(System.AppDomain.CurrentDomain.BaseDirectory, entityTypeAssemblyName);
                }
        ...
        public static System.Data.DbType Type2Dbtype(Type type)
        {
        ...
        public static Type DbType2Type(System.Data.DbType dbType)
        {
